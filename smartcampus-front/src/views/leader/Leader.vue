    ...
    <header class="header">
      <div class="header-inner">
        ...
        <nav class="top-nav">
          ...
          <a href="#" @click.prevent="router.push('/leader/announcement')" class="nav-item">通知公告</a>
          <a href="#" @click.prevent="router.push('/leader/news')" class="nav-item">学校新闻</a>
        </nav>
        <div class="user-area">
    ...
    <template v-if="route.path === '/leader'">
      <div class="main-wrap">
        <div class="main-inner">
          <div class="welcome-hero leader-hero">
            <div class="hero-content">
              <h1>{{ userStore.userInfo?.name || '领导' }}，欢迎回来</h1>
          ...
          <div class="info-row">
            <div class="info-panel">
              <div class="panel-head">
                ...
                <a href="#" @click.prevent="router.push('/leader/announcement')" class="panel-more">更多 →</a>
              </div>
              <ul class="news-list" v-if="announcements.length">
                <li v-for="item in announcements.slice(0,5)" :key="item.id" @click="router.push('/leader/announcement')" class="news-item">
            ...
            <div class="info-panel">
              <div class="panel-head">
                ...
                <a href="#" @click.prevent="router.push('/leader/news')" class="panel-more">更多 →</a>
              </div>
              <ul class="news-list" v-if="news.length">
                <li v-for="item in news.slice(0,5)" :key="item.id" @click="router.push('/leader/news')" class="news-item">
      ...
      <footer class="footer">
        <p>© 2026 智慧校园服务平台 · {{ userStore.userInfo?.department||'校办' }}</p>
      </footer>
    </template>

    <div v-else class="sub-page">
      <div class="sub-nav"><a href="#" @click.prevent="router.push('/leader')" class="back-link">← 返回首页</a></div>
      <router-view />
    </div>
  </div>
</template>
...
const services = [
  { icon:'👤', label:'个人信息', path:'/leader/profile' },
  { icon:'📒', label:'通讯录',   path:'/leader/directory' },
  { icon:'📋', label:'工作计划', path:'/leader/work-plans' },
  { icon:'📄', label:'公文审批', path:'/leader/documents' },
  { icon:'✅', label:'奖助贷审批', path:'/leader/applications' },
  { icon:'🔬', label:'科研总览', path:'/leader/research' },
  { icon:'📊', label:'数据统计', path:'/leader/stats' },
]
...
.logo-text { font-size:17px; font-weight:700; color:#b8860b; letter-spacing:1px; }
...
.top-nav .nav-item:hover, .top-nav .nav-item.active { color:#b8860b; background:#fef9ed; }
...
.welcome-hero { background:linear-gradient(135deg,#b8860b 0%,#d4a745 50%,#8b6914 100%); border-radius:8px; padding:28px 32px; margin-bottom:20px; color:#fff; }
...
.panel-more { font-size:12px; color:#b8860b; text-decoration:none; }
...
.service-item:hover { background:#fdf8ee; }
...
.back-link { font-size:13px; color:#b8860b; text-decoration:none; }
</style>
